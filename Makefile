...
CC := gcc
CFLAGS := -O2 -fPIC -I$(LUA_DIR) -I$(LIBUV_DIR)/include
LDFLAGS := -L$(LUA_DIR) -L$(LIBUV_DIR)/build/Release -llua -luv
...
ifeq ($(OS),Windows_NT)
	...
	@cd $(LIBUV_DIR) && ./autogen.sh && ./configure && make && make install
endif
